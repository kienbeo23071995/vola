...
const UpdateMedia = () => {
    ...
    const mediaCreate = async (e) => {
        ...
        await axios.post("http://localhost:8080/editMedia/" + params.idcampaign + "/" + params.id, formData)
            .then(response => {
    ...
    useEffect(() => {
        (async () => {
            await axios.get("http://localhost:8080/getByIdCampagin/" + params.idcampaign).then(response => {
                var data = response.data.find(n => n.id == params.id);
                setSelectedOption(params.idcampaign + "");
    ...
    return (
        <div className="create_account">
            <h1 style={{ borderRadius: 1, marginTop: 40, marginBottom: 0, padding: 15, color: "#FFFFFF", background: "linear-gradient(to right, #2D754E, #5E9F6B, #74C99B)" }}>Sửa Media</h1>
